...
class tower(private val context: Context, var x: Float, var height: Int) {
    ...
    private val gap: Int = 800
    var passed = false

    fun draw(canvas: Canvas) {
        val topRect = Rect(x.toInt(), 0, (x + width).toInt(), height)
        val bottomRect = Rect(x.toInt(), height + gap,
            (x + width).toInt(), canvas.height)
    ...
    fun update() {
        ...
        if (x < -width) {
            ...
            height = (Math.random() *
                    (context.resources.displayMetrics.heightPixels - gap)).toInt()
        }
    }
    ...
    fun getBottomRectangle(): Rect {
        return Rect(x.toInt(), height + gap, (x + width).toInt(),
